...
No MetaTrader5 SDK objects are allowed to leave this adapter.
Everything returned to the remainder of AEGIS must be translated into
the standard domain models under app.schemas.
...
class MT5Adapter(BrokerAdapter):
    ...
    # ==========================================================
    # INTERNAL HELPERS - SDK ISOLATION
    # ==========================================================

    def _map_position_to_model(self, mt5_pos) -> Position:
        """Convert MT5 position tuple to Position pydantic model"""
        return Position(
    ...
    def _map_order_to_model(self, mt5_order) -> PendingOrder:
        """Convert MT5 order tuple to PendingOrder pydantic model"""
        type_map = {
        ...
        return PendingOrder(
            ...
            order_type=type_map.get(mt5_order.type, "unknown"),
            volume=mt5_order.volume_current,
            price_open=mt5_order.price_open,
